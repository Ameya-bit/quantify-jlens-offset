...
- [x] **4.0 Noise null FIRST:** N ≈ 500–2000 (prompt, position) pairs, positions ≥ 4, diverse pile prompts. Split into two disjoint halves; compute `m_t` (mean score per token per layer) on each; correlate. **No replication ⇒ no stable offset ⇒ all three public claims fail at once — that is the writeup.** → **GATE PASSED, pre-registered criterion (D34): min J-lens split-half Pearson 0.9589 over layers (R 0.9625, logit 0.9570; Spearman ≥0.95 all three); 100 seed-2 docs (row-disjoint from seed-0/seed-1 samples, asserted), 1,987 samples, document-level halves. Pythia same design: 0.9902.** Caveat: top-100 *identity* overlap between halves only 0.56–0.87 — tail-membership claims are noisy, vector-level claims are not. (`src/offset_battery.py` → `results/step4_mt.npz`, `step4_noise_null.json`; Pythia: `src/offset_battery_pythia.py` → `step4_mt_pythia.npz`.)
- [x] `m_t`, `σ_t` per layer on Qwen (and Pythia when fit lands). High-m/low-σ = offset; high-σ = content. (Both elevated = multiplicative/unembedding-norm signature — Neel's own aside; flag if seen.) → **SEEN and flagged — in the transported lenses: top-100 m_t tokens sit at the 93rd–99th σ_t percentile every layer on R, 85th–99th on J; the LOGIT lens sits at only the 54th–66th across L5–18 (corrected 24 Aug red team — first version wrongly said "91st–99th at every layer/instrument"). Mid-depth logit lens looks additive, consistent with Route A; the multiplicative signature is a property the transport adds.** Composition: J L0 = `\xa0`/soft-hyphen/`�`/code fragments (junk share 0.22), L12 = ` thru`/`...`/whitespace runs (the Δ_t web-debris axis), L24 = function words. (`src/mt_diagnostics.py` → `results/step4_mt_diagnostics.json` + cached per-token flags `step4_token_flags.npz`.)
...
- [x] **4c H1 regression:** Spearman/OLS of `m_t` on `log f_t` per layer; r vs depth. Headline figure's main line. → **Mid-depth (L5–28), J-lens (R matches): all three cells agree, POSITIVE (frequency-prior-like), peak L18–21 at ρ = 0.48 / 0.25 / 0.14 — the D31 range in action. Early layers (L0–4), where the junk lives: cells DISAGREE in sign and every |ρ|<0.12 — frequency does not structure the junk. **Against the registered H1 (frequency prior, plan v2): SUPPORTED at mid-depth for the transported lenses (J/R)**; the early-layer junk is a separate, frequency-independent component of the offset (framing corrected 21 Aug after review — devlog 0.1.0 correction section). Instrument scope disclosed 24 Aug (red team): the Qwen LOGIT lens tilts NEGATIVE at mid/late depth (Latin −0.05 L18 → −0.33 L28) — the frequency prior is a transported-lens property on Qwen; on Pythia both instruments are positive at every layer (J 0.16–0.50, logit 0.29–0.64, `results/step4_h1_regression_pythia.json`), so the negative tilt is Qwen-specific. Centering→rarer now measured directly, confirmed every J layer (`results/step4_centering_freq.json`). Bare-word restriction (D33) shifts ρ by ≤0.05, sign flips only where |ρ|<0.02. One headline number re-derived independently without scipy: exact match; full 4c rerun bit-identical (24 Aug).** Interpretive limit stated: positive m_t–frequency correlation cannot separate instrument bias from genuine content. (`src/h1_regression.py` → `results/step4_h1_regression.json` + `step4_h1_r_by_depth.png`; refuses to run on a failed 4.0 gate.)
