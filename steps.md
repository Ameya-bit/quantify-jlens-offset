...
- [x] Setup figure: junk (obscene/CJK/punctuation/rare fragments) at early layers, fading with depth — J-lens vs R-lens vs logit-lens side by side. (`results/step2_junk_fraction.png` + grid figures. J junk ≈0.3 early → 0.08 late; junk present in all three instruments. Initial "logit junkier than J/R" reading **retracted** — flag-proxy recorder bias (counts CJK, blind to Latin-fragment junk; see devlog 0.0.2 addendum + `results/step2_junk_composition.json`). Lens-vs-lens junk comparison deferred to behavioral m_t (Step 4/6). Bonus: `results/step2_overlap_L30.png` replicates the paper's late-layer-agreement claim on our rig.)
- [x] Background: Pythia-1.4B J-lens fit via official `jlens.fit` — **n raised 10→25** (smoke test: 101 s/text, `results/step2_pythia_smoke.json`; 25 matches released recipe). Launched, checkpointed. Precondition before Step 4 uses it: pass the Step-1-style sanity battery. → **PASSED 3/3** 20 Aug (`results/pythia_sanity.json`; caveat: late-layer J~logit agreement 5/10 at L21, weaker than released-Qwen analog — disclosed in devlog 0.0.2).
