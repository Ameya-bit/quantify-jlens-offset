"""Sanity battery for OUR self-fitted gemma-2-9b-it J-lens (Gemma-arm gate).

Mirror of src.sanity_qwen3 (same checks, same gates), layer indices shifted
for Gemma-2-9B's 42 layers: fitted 0-40 (destination 40 -> fitted 0-39),
agreement at the last three fitted layers, gate at the last one. Wiring
check exercises jlens's final-logit-softcapping path against the model's
true logits.

Run: python -m src.sanity_gemma2   (writes results/taboo_gemma/gemma2_sanity.json)
"""
...
OUT_PATH = "results/taboo_gemma/gemma2_sanity.json"
...
def main() -> None:
    ...
    agreement_layers = fitted_layers[-3:]
    gate_layer = fitted_layers[-1]
